(() => {
  ...
  const translations = {
    en: {
      ...
      "マイクが捉えるのは、すでに音になった自然です。SURFACE / SIGNALが観測するのは、その手前にある動き。風に揺れる葉、水面を走る波紋、海のうねりを、変位の時系列として読み取ります。": "A microphone captures nature after it has become sound. SURFACE / SIGNAL observes the motion that comes before it: leaves moving in the wind, ripples crossing water, and ocean swells, read as time-series displacement.",
      "SURFACE / SIGNALは、水面、ToFセンサ、信号処理をひとつの": "SURFACE / SIGNAL combines a water surface, a ToF sensor, and signal processing into a single ",
      ...
      "振動体である水面、単一点の変位を読むToFセンサ、波形を可聴化するプロセッサ。この三者を統合した系全体を、SURFACE / SIGNALでは「物理–計算オシレーター」と呼びます。": "SURFACE / SIGNAL calls the complete system a physical–computational oscillator: the water surface as vibrating body, a ToF sensor reading displacement at one point, and a processor making the waveform audible.",
      ...
      "一般的なシンセサイザでは、内蔵オシレーターがサイン波・ノコギリ波・矩形波などを生成します。SURFACE / SIGNALでは、その音源位置へ": "A conventional synthesizer uses internal oscillators to generate sine, sawtooth, square, and other waves. SURFACE / SIGNAL instead supplies ",
    ...
    zh: {
      ...
      "マイクが捉えるのは、すでに音になった自然です。SURFACE / SIGNALが観測するのは、その手前にある動き。風に揺れる葉、水面を走る波紋、海のうねりを、変位の時系列として読み取ります。": "麦克风捕捉的是已经成为声音的自然。SURFACE / SIGNAL 观测的是声音之前的运动：风中的树叶、水面的涟漪与海浪，都被读取为位移的时间序列。",
      "SURFACE / SIGNALは、水面、ToFセンサ、信号処理をひとつの": "SURFACE / SIGNAL 将水面、ToF 传感器与信号处理构成一个",
      ...
      "ひとつの発振系に。": "组成一个振荡系统。",
      "振動体である水面、単一点の変位を読むToFセンサ、波形を可聴化するプロセッサ。この三者を統合した系全体を、SURFACE / SIGNALでは「物理–計算オシレーター」と呼びます。": "水面是振动体，ToF 传感器读取单点位移，处理器将波形转化为可听声音。SURFACE / SIGNAL 将三者整合而成的整体称为“物理–计算振荡器”。",
      ...
      "一般的なシンセサイザでは、内蔵オシレーターがサイン波・ノコギリ波・矩形波などを生成します。SURFACE / SIGNALでは、その音源位置へ": "传统合成器由内置振荡器生成正弦波、锯齿波、方波等。SURFACE / SIGNAL 则在这一声源位置提供",
  ...
  const metadata = {
    ja: {
      title: "SURFACE / SIGNAL — Water Surface Sonification Study",
      description: "水面、ToFセンサ、信号処理を物理–計算オシレーターとして構成し、波紋から音源グレインを生成する研究 SURFACE / SIGNAL。"
    },
    en: {
      title: "SURFACE / SIGNAL — Water Surface Sonification Study",
      description: "SURFACE / SIGNAL combines a water surface, ToF sensing, and signal processing into a physical–computational oscillator that generates source grains from ripples."
    },
    zh: {
      title: "SURFACE / SIGNAL — 水面声音化研究",
      description: "SURFACE / SIGNAL 将水面、ToF 传感与信号处理构成物理–计算振荡器，从涟漪中生成声源颗粒。"
    }
  };
  ...
  function setLanguage(language) {
    ...
    try { localStorage.setItem("surface-signal-language", language); } catch (error) {}
  ...
  try { initialLanguage = localStorage.getItem("surface-signal-language") || "ja"; } catch (error) {}
